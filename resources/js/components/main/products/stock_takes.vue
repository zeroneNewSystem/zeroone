      ...
      <v-card-text>
        ...
        <v-row>
          <v-data-table
            ...
            :items="stocktake.stock_take_details"
            class="elevation-1"
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.stocktake.stock_take_details.splice(
        this.stocktake.stock_take_details.indexOf(item),
        1
      );
    },
    product_unit_change(item) {
      ...
      item.current_quantity = parseInt(
        item.purchase_details[0].quan_in_mininmal_unit /
    ...
    getProducts(val, type) {
      if (val.length > 2) {
        ...
        let params = "";
        if (type == "barcode")
          params = { barcode: val, inventory_id: this.stocktake.inventory_id };
        else params = { name: val, inventory_id: this.stocktake.inventory_id };
    ...
    showThisProduct(selected_product) {
      if (
        this.stocktake.stock_take_details.findIndex(
      ...
      selected_product.current_quantity =
        selected_product.purchase_details[0].quan_in_mininmal_unit /
...
selected_product.actual_quantity = selected_product.current_quantity
    ...
    searchAndAddToStockTackes() {
      ...
      Product.stockTakeBarcodeSearch(params).then((response) => {
        ...
        if (products_grouped) {
          this.selected_item.purchase_details[0].quan_in_mininmal_unit =
            this.selected_item.purchase_details.reduce(
              (a, b) => +a + +b.quan_in_mininmal_unit,
              0
            );
