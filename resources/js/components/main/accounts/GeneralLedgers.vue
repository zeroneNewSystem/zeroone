  ...
  <div>
    ...
    <v-row class="ma-2" style="font-size: small; border: 1px solid lightgrey">
      <v-col>
        ...
        <v-row>
          <v-col>المجموع</v-col>
          <v-col>{{ debit_sum().toFixed(2) }}</v-col>
          <v-col>{{ credit_sum().toFixed(2) }} </v-col>
          <v-col>{{ debit_sum().toFixed(2) + credit_sum().toFixed(2) }}</v-col>
        </v-row>
      </v-col>
    </v-row>
  </div>
...
export default {
  ...
  methods: {
    getGeneralLedgers() {
      Account.getGeneralLedgers(this.search).then((response) => {
        this.query = false;
        this.show = false;
