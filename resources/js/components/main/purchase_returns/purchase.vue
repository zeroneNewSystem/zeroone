...
export default {
  ...
  methods: {
    logo(item) {
      ...
      for (
        let index = 0;
        index < this.purchase.details.length;
        index++
      ) {
        if (
          this.purchase.details[index].barcode == item.barcode &&
          this.purchase.details[index].expires_at == item.expires_at
        ) {
          if (firstIndex == -1) firstIndex = index;
          occurrences++;
          if (occurrences == 2) {
            this.purchase.details[firstIndex].quantity +=
              this.purchase.details[index].quantity;
    ...
    searchAndAddToPurchase() {
      ...
      Product.purchaseBarcodeSearch(params).then((response) => {
        ...
        if (!selected_product.has_expiration_date) {
          let index = this.purchase.details.findIndex(
        ...
        this.purchase.details.push(selected_product);
      });
    },
    remaining_amount() {
