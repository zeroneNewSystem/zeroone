          ...
          <v-container>
            ...
            <v-row>
              ...
              >
                  ...
                  <v-toolbar flat color="white">
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-autocomplete
                        ...
                        hide-no-data
                        label="اسم الصنف"
                        @change="addProductToBill_return"
                      ></v-autocomplete>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field
                        ...
                        label="الباركود"
                        @keydown.enter="searchAndAddToBill_return"
                        :rules="is_exists"
                      ></v-text-field>
                    </v-col>
                ...
                <template v-slot:item.bill_tax="{ item }">
                  <v-text-field
                    type="number"
                    flat
                    hide-no-data
                    ...
                    outlined
                    autocomplete="off"
                    v-model="item.bill_tax"
                  ></v-text-field>
                </template>
                <template v-slot:item.bill_tax_value="{ item }">
                  <v-text-field
                    flat
                    disabled
                    hide-no-data
                    ...
                    outlined
                    autocomplete="off"
                    :value="bill_tax_value(item).toFixed(2)"
                  ></v-text-field>
                </template>

                <template v-slot:item.bill_discount="{ item }">
                  <v-row class="justify-center">
                    <v-col cols="6" class="pl-0">
                      <v-text-field
                        ...
                        outlined
                        autocomplete="off"
                        v-model="item.bill_discount"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="6" class="pr-0">
                      <v-autocomplete
                        v-model="item.bill_discount_type_id"
                ...
                <template v-slot:item.billd_quantity="{ item }">
                  <v-text-field
                    type="number"
                    hide-no-data
                    hide-details
...
import Bill_return from "../../../apis/BillReturn";
  ...
  methods: {
    logo(item) {
      ...
      for (
        ...
        if (
          this.bill.bill_details[index].barcode ==
            item.barcode &&
          this.bill.bill_details[index].expires_at ==
            item.expires_at
        ) {
      ...
      Product.billBarcodeSearch(params).then((response) => {
        ...
        selected_product.unit_id =
          selected_product.units[
            selected_product.main_bill_unit_id - 1
          ].pivot.id;
        ...
        selected_product.billd_quantity = 1;
    ...
    payAllCash() {
      this.bill.paid_amount =
        this.bill.total_amount.toFixed(2);
    ...
    unit_change(item) {
      let billd_unit = item.units.find(
        (elem) => elem.pivot.id == item.unit_id
      );

      item.unit_price = billd_unit.pivot.bill_price;
    },
    total_vat() {
      this.bill.total_vat =
        this.bill.bill_details.reduce(
          (a, b) => +a + +b.bill_tax_value,
          0
        );
    ...
    total(item) {
      item.total =
        this.bill_tax_value(item) + this.total_befor_tax(item);
      return item.total;
    },

    bill_tax_value(item) {
      item.bill_tax_value =
        (this.total_befor_tax(item) * item.bill_tax) / 100;
      return item.bill_tax_value;
    },
    total_befor_tax(item) {
      if (item.bill_discount_type_id == 1) {
        item.total_befor_tax =
          item.billd_quantity * item.unit_price -
          (item.billd_quantity *
            item.unit_price *
            item.bill_discount) /
            100;
    ...
    quantity_in_minor_unit(item) {
      ...
      item.quantity_in_minor_unit =
        item.billd_quantity * billd_unit.pivot.contains;
    ...
    addProductToBill_return() {
      ...
      this.selected_product.billd_quantity = 1;
    ...
    submit() {
      ...
      // );
      if (this.is_new_bill)
        Bill_return.store(this.bill).then((response) =>
          console.log(response.data)
        );
      else
        Bill_return.update(this.bill).then((response) =>
          console.log(response.data)
        );
    ...
    async createPage(to, status) {
      ...
      if (Object.keys(params).length != 0) {
        ...
        Bill_return.get(params.id).then((response) => {
          this.bill = response.data.bill;
          console.log(this.bill);
          this.bill.issue_date =
            this.bill.issue_date.split(" ")[0];
        ...
        if (status == "new") {
          ...
          this.bill = JSON.parse(
            JSON.stringify(this.new_bill)
          );
